...
% load the images of the water+CuSO4 phantom
load h2o_images;%5 psis, 18 flip angles, matrix 100 x 100
...
for p=1:5
    for k=1:18
        if p<5
            image_cut=ROI.*squeeze(h2o_images(p,k,:,:));
        else
            image_cut=ROI169.*squeeze(h2o_images(p,k,:,:));
        end
        signal(p,k)=sum(image_cut(:));
    end
    signal(p,:)=signal(p,:)./max(signal(p,:));
end

%show the ROI
figure;imagesc((1-ROI).*squeeze(h2o_images(1,2,:,:)));
