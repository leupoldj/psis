%Simulation of Fig 3 and Fig 4.
%The only difference between them is the TR.
%For Fig3, use TR=20, for Fig 4, use TR=50

TR=20 %change this to TR=50 for Fig. 4
...
% calculate the ernst amplitude at the ernst angle
for t1=1:length(T1_vec)
    T1=power(10,T1_vec(t1));
    ernst_angle=acosd(exp(-TR/T1));
    ernstmax(t1)=sind(ernst_angle)*(1-exp(-TR/T1))/(1-exp(-TR/T1)*cosd(ernst_angle));
end
