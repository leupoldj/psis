function [S_plus, S_minus] = epg_rfsp(alpha,np,T1,T2,TR,D,psi,res)
...
for l = 0:(np-1)   % RF pulse loop
    ...
    if (l>0)
    S_minus(l+1) = fx(np+1)+1i*fy(np+1);  %function output: the pre pulse signal                                          
    end
    ...
    S_plus(l+1) = pfx(np+1)+1i*pfy(np+1); %function output: the post pulse signal
